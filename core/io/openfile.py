...
def open_file():
    local_dir="D:/sc/work_demo/etl/1837/files"
    for file in os.listdir(local_dir):
        path = '/'.join([local_dir, file])
        new_file_name=hashlib.sha1(file).hexdigest()
        new_path = '/'.join([local_dir, new_file_name])
        print path,new_path
        os.rename(unicode(path,'utf-8'),new_path)
